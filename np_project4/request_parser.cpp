...
request_parser::request_parser()
  ...
  //parse query string
  void request_parser::parseQueryString(char* data, std::vector<struct shell_request>& shell_requests, std::string &socks_host, std::string &socks_port) {
    ...
    while(token != NULL) {
      ...
      switch(token[0]) {
        ...
        case 's':
          //socks host
          if(token[1] == 'h') {
            socks_host = std::string(token+3);
          }
          //socks port
          else if(token[1] == 'p') {
            socks_port = std::string(token+3);
          }
          break;
        default:
          break;
